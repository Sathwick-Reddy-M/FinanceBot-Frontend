...
import { PlusCircle, Briefcase, Landmark, TrendingUp, CreditCardIcon, Layers, Wallet } from 'lucide-react'; // Specific icons
        ...
        {ACCOUNT_TYPES.map(type => (
          <div key={type}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {[1,2].map(i => <Skeleton key={`${type}-skel-${i}`} className="h-64 rounded-xl" />)}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-10">
      ...
      {ACCOUNT_TYPES.map((type) => {
        ...
        if (accountsOfType.length === 0 && !accounts.some(acc => acc.type === type && accountsOfType.length > 0)) { 
           // Optionally render a message if you want to show empty sections always
           // return (
           //   <section key={type} className="space-y-4 p-4 bg-card rounded-xl shadow-sm border">
           //     <h3 className="text-2xl font-semibold text-primary border-b-2 border-primary/20 pb-2 flex items-center">
           //       <AccountTypeIcons[type] className="mr-3 h-6 w-6" />
           //       {type} Accounts
           //     </h3>
           //     <p className="text-muted-foreground">No {type.toLowerCase()} accounts added yet.</p>
           //   </section>
           // );
           return null; // Don't render section if no accounts of this type
        }
        ...
        return (
          (accountsOfType.length > 0 ) && ( 
            <section key={type} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6"> {/* Max 2 cards per row in this section */}
